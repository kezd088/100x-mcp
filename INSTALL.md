...
5. 在新任务中调用 `100x_connect`，打开返回的 100x 授权页面，将连接码展示给用户核对。用户自行登录并点击允许，Agent 不代替用户授予账户权限。按返回间隔检查 `100x_connection_status`，只有 `connected` 才算完成。Windows 备选入口是安装器 `-Connect`。不要索取令牌到聊天。
...
如果交给 Codex 执行，省略 `-Connect`，在新任务中发送「连接 100x」，由用户在网页确认授权。之后的生成都在桌面端对话中进行。
...
macOS / Linux 的加密凭据向导尚未提供和验收。开发者可通过 `HUNDREDX_URL`（完整 `/api/mcp` 地址）及 `HUNDREDX_TOKEN` 环境变量接入，但桌面端必须实际继承这些变量；终端临时赋值不等于 GUI 已配置。普通桌面用户当前以 Windows 安装向导为准。

## 状态与更新

v0.3.0 默认连接 `https://100xspeed.app/api/mcp`，通过 100x 网页授权当前设备。安装不赠送积分。生产模型、规格与费用以 100x 实时目录及报价为准。更新可运行 `codex plugin marketplace upgrade 100x` 后重新执行安装器。卸载执行 `codex plugin remove 100x@100x`；加密凭据不会随之删除，需要清除时由用户自行管理 `%LOCALAPPDATA%/100x/connection.json` 及其备份。
